    ...
    ERC1155TokenReceiver
{
    ...
    // rewards token(Tangent token now)
    ERC20Interface public TangentToken;

    // lp token about
    // The stake balances of users, it will contains interest(user address->amount), input token is Tangent
    ...
    constructor(address _TangentAddress, address _lpAddress) {
        require(
            _TangentAddress.isContract() && _lpAddress.isContract(),
            "must use contract address"
        );
        TangentToken = ERC20Interface(_TangentAddress);
    ...
    /**
     * Store farming rewards to TangentStakingFarm contract, in order to pay the user interest later.
     *
    ...
    function _stake(address _sender, uint256 _amount) internal {
        ...
        // calculate rewards of Tangent token
        uint256 TangentInterest = calculateTangentTokenRewards(_sender);
        ...
        // Transfer TangentToken interest to user
        transferTangentInterest(_sender, TangentInterest);
    }
    
    /**
     * Transfer TangentToken interest to user.
     */ 
    function transferTangentInterest(address recipient, uint256 amount) internal {
        if (amount <= 0) {
            return;
        }
        ...
        require(
                TangentToken.transfer(recipient, amount),
                "transfer Tangent interest failed"
            );
    }
    ...
    function _unstake(address _sender) internal {
        ...
        // calculate total balance with interest(the interest is Tangent token)
        (uint256 totalWithInterest, uint256 principalOfRepresentTangent, uint256 timePassed) =
        ...
        uint256 interest = totalWithInterest.sub(principalOfRepresentTangent);
    ...
    function _stakeNft(
        ...
        // calculate rewards of Tangent token
        uint256 TangentInterest = calculateTangentTokenRewards(_from);
        ...
        // Transfer TangentToken interest to user
        transferTangentInterest(_from, TangentInterest);
        // send event
    ...
    function _batchStakeNfts(
        ...
        // Transfer TangentToken interest to user
        transferTangentInterest(msg.sender, TangentInterest);
        // send event
    ...
    function _unstakeNft(
        ...
        // calculate rewards of Tangent token
        uint256 TangentInterest = calculateTangentTokenRewards(msg.sender);
        // update stakeDate of user
    ...
    */
    function claim() external whenNotPaused nonReentrant {
        ...
        (uint256 totalWithInterest, uint256 principalOfRepresentTangent, uint256 timePassed) = calculateRewardsAndTimePassed(msg.sender, 0);
        require(
            totalWithInterest > 0 && timePassed >= 0,
    ...
    /**
     * Calculate user's TangentToken rewards.
     *
     * @param _from User address.
     */
    function calculateTangentTokenRewards(address _from) public view returns(uint256) {
        // if lpToken balance>0, pass time > 1 seconds, should calculate rewards of TangentToken.
        ...
        // calculate total balance with interest
        (uint256 totalWithInterest, uint principalOfRepresentTangent, uint256 timePassed) =
        ...
        return totalWithInterest.sub(principalOfRepresentTangent);
    }
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        (uint112 TangentReserve,,) = lpToken.getReserves();
        uint256 principalOfRepresentTangent = Calculator.getValueOfRepresentTangent(amount, lpTokenTotalSupply, TangentReserve);
        uint256 totalWithInterest =
            Calculator.calculator(principalOfRepresentTangent, timePassed, totalApy);
        return (totalWithInterest, principalOfRepresentTangent, timePassed);
    }
     ...
     */
    function getTangentBalance(address addr) public view returns (uint256) {
     ...
     *
     * Note: when Tangent token staked, base apy will be 12%; otherwise total apy will be 0.
     *
     * @param user The address of user.
     */
    function getTotalApyOfUser(address user) public view returns (uint256) {
        uint256 balanceOfTangent = balances[user];
        // if Tangent balance=0, the apy will be 0
        if (balanceOfTangent <= 0) {
            return 0;
        }
        // totalApy
