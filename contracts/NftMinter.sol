...
contract NftMinter is ERC1155Burnable, ReentrancyGuard, Pausable, Ownable {
    ...
    // TangentToken(use TangentToken to pay minting fee)
    ...
    constructor(
        ...
        address _TangentAddress
    ) ERC1155("") {
        require(
            _TangentAddress.isContract(),
            "_TangentAddress must be contract address"
        );
        name = _name;
        symbol = _symbol;
    ...
    function mint(
        ...
        for (uint256 i = 0; i < _fees.length; i++) {
            ...
            uint256 TangentAmount = SafeMath.div(
                SafeMath.mul(_fees[i].percent, mintingFee),
                100
            );
            require(TangentAmount > 0, "Tangent mintingFee should bigger than 0");
            // send fee to Tangent.DIGITAL DEV fund and Tangent Farming Pool
            require(
                TangentToken.transferFrom(
                    msg.sender,
                    _fees[i].recipient,
                    TangentAmount
                ),
