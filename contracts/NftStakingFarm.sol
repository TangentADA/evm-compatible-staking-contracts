    ...
    ERC1155TokenReceiver
{
    ...
    // other constants
    // base APY when staking just ERC20 Tangent token is 12%, only contract owner can modify it
    ...
    constructor(address _tokenAddress) {
        ...
        TangentToken = ERC20Interface(_tokenAddress);
    ...
    function _stake(address _sender, uint256 _amount) internal {
        ...
        // recalculate balance of Tangent token, then stake Tangent token and update the stake date
        ...
        // transfer Tangent token to contract
        require(
            TangentToken.transferFrom(msg.sender, address(this), _amount),
            "transfer failed"
        );
    ...
    function _unstake(address _sender) internal {
        // get Tangent token balance of current user
        ...
        if (totalFunding >= interest) {
            ...
            // total funding is not enough to pay interest, the contract's Tangent has been completely drained.
        ...
        require(
            TangentToken.transfer(_sender, unstakeAmount),
    ...
    function _stakeNft(
        ...
        // recalculate balance of Tangent token
    ...
    */
    ...
    function recalculateBalance(address _from) internal {
        // if Tangent balance>0, should update user's Tangent balance, and update stake date
        // get current Tangent token balance
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 totalWithInterest =
            Calculator.calculator(amount.mul(lpPrice), timePassed, totalApy);
    ...
    /**
     * Update price of lpToken.
     *
     * Requirements: the caller must be the owner.
     */
    function updateLpPrice(uint256 newLpPrice) public onlyOwner {
        lpPrice = newLpPrice;
    }


    /**
     * Withdraws all total funded tokens.
     *
     * Requirements: the caller must be the owner.
     */
    function withdrawFunding(uint256 amount) public onlyOwner {
        if (totalFunding >= amount) {
            totalFunding = totalFunding.sub(amount);
            require(
                TangentToken.transfer(msg.sender, amount),
